...
trisk <- c(0,3,6,9,12,15,18,21,24,27,30,33,36,39,42,45,48,51)

## number at risk experimental arm (no space allowed)
nrisk.E <- c(342,183,100,70,58,49,47,43,36,28,26,21,18,13,6)

## number at risk control arm (no space allowed)
nrisk.C <- c(332,137,74,39,30,22,17,10,8,7,5,3,2,1,1)
...
### calculate percentage censored at each time interval in exp
est_E_OS$riskmat$percentage_censor_hat_rounded <- round((est_E_OS$riskmat$censor.hat / est_E_OS$riskmat$nrisk.hat) * 100, 1)

### calculate percentage censored at each time interval in exp
est_C_OS$riskmat$percentage_censor_hat_rounded <- round((est_C_OS$riskmat$censor.hat / est_C_OS$riskmat$nrisk.hat) * 100, 1)

print(est_E_OS$riskmat$percentage_censor_hat_rounded)

print(est_C_OS$riskmat$percentage_censor_hat_rounded)
